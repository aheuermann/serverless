...
module.exports = {
  isApplicable({ options, serviceDir }) {
    ...
    if (serviceDir && Object.keys(options).some((key) => notApplicableOptions.has(key))) {
      throw new ServerlessError(
        'Cannot setup a new service when being in context of another service ("--name", "--template-path", "--template" and "--template-url" options cannot be applied)',
  ...
  async run(context) {
    ...
    if (Object.keys(context.options).filter((key) => templateOptions.has(key)).length > 1) {
      throw new ServerlessError(
        'You can provide only one of: "--template-path", "--template" and "--template-url" options',
    ...
    if (context.options['template-path']) {
      ...
      try {
        process.stdout.write(
          `\n${chalk.green(`Downloading template from provided url: ${templateUrl}...`)}\n`
        );
        await downloadTemplateFromRepo(templateUrl, null, projectName, { silent: true });
      } catch (err) {
        throw new ServerlessError(
          'Could not download template from provided url. Ensure that the template provided with "--template-url" exists.',
